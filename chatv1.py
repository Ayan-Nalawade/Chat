...
import time
import requests, re, json, random, time, queue, threading, traceback, hashlib, string, random
...
for each in sys.path:
  if os.path.exists(f"{each}/poe_graphql"):
    parent_path = Path(each)
    break
if parent_path == "":raise Exception("Sorry, Python Home Installation file not found!")
else: print(f"Using {parent_path} for installing additional dependencies, if required")
...
class Client:
  ...
  def __init__(self, token, proxy=None, headers=headers):
    ...
    self.adapter = requests.adapters.HTTPAdapter(pool_connections=100,
                                                 pool_maxsize=100)
  ...
  def get_bots(self, download_next_data=True):
    ...
    for bot in bot_list:
      thread = threading.Thread(target=get_bot_thread,
                                args=(bot, ),
                                daemon=True)
  ...
  def get_remaining_messages(self, chatbot):
    ...
    return chat_data["defaultBotObject"]["messageLimit"][
  ...
  def send_query(self, query_name, variables, attempts=20):
    for i in range(attempts):
      ...
      base_string = payload + self.gql_headers[
        "poe-formkey"] + "WpuLMiXEKKE98j56k"
      ...
      r = request_with_retries(self.session.post,
                               self.gql_url,
                               data=payload,
                               headers=headers)
  ...
  def connect_ws(self):
    ...
    self.ws = websocket.WebSocketApp(self.get_websocket_url(),
    ...
    try:
      ...
      for message_str in data["messages"]:
        ...
        for key, value in copied_dict.items():
          ...
          elif key != "pending" and value == None and message[
              "state"] != "complete":
  ...
  def get_message_history(self, chatbot, count=25, cursor=None):
    ...
    cursor = str(cursor)
    if count > 50:
      messages = self.get_message_history(chatbot, count=50,
                                          cursor=cursor) + messages
      while count > 0:
        count -= 50
        new_cursor = messages[0]["cursor"]
        new_messages = self.get_message_history(chatbot,
                                                min(50, count),
                                                cursor=new_cursor)
...
if len(sys.argv) == 2 and sys.argv[1].lower() == "--no-logs":
  logpolicy = False
  print(f"{Fore.LIGHTYELLOW_EX} No logs turned on!")

if os.path.exists("./chat.context") and logpolicy == True:
  with open("chat.context", "rb") as index:
    data2 = pickle.load(index)

def retrieve_messages(channel_id: int) -> None:
    headers = {
        'authorization': 'ODU3MzI1NDA0MzE4NzI4MjEy.GcUuZY.sEKt9AixDMrPsvdkoXJJKHUIlX8mXo8dMVIWU0'
    }
...
def pr_botnames(local_client) -> None:
  ...
  for key, value in local_client.bot_names.items():
    print(f"{Fore.CYAN} {count}. {key} : {value}")
    count += 1
...
while True:
  ...
  if ask.lower() == "exit":
      ...
      break
  elif ask.lower() == f"switch {ask[7:]}":
      client.purge_conversation(model)
      model = (ask[7:]).lower()
      continue
  elif ask.lower() == "clear context" or ask.lower() == "cls_context":
      client.purge_conversation(model)
      if data2 != "" and logpolicy == True:data2 = ""
  ...
  print(f"Using {Fore.BLUE}{model}")
  
  try:
    for chunk in client.send_message(model, ask):
        if logpolicy == True:
          tmp += chunk["text_new"]
        print(Fore.WHITE + Back.BLACK + chunk["text_new"],
            end="",
            flush=True)
  except RuntimeError:
      for i in range(0,10000):
        print(f"\r {Fore.YELLOW} Current token: {tokens[count]}", end='')
        try:
          client = Client(tokens[count])
          for chunk in client.send_message(model, ask):
            if logpolicy == True:
              tmp += chunk["text_new"]
            print("\n" + Fore.WHITE + Back.BLACK + chunk["text_new"],
            end="",
            flush=True)
          break
        except Exception:
